...
function processFile(sourcePath, templates, customPartialName) {
  return (file, callback) => {
    const layout = templates.layouts[file.meta.layout];
    const fns = wrapFunctions(templates.fns, file);
  ...
  return (files, callback) => {
    async.waterfall([
      ...
      (templates, cb) => async.map(
        files,
        processFile(sourcePath, templates, customPartialName),
        cb
      ),
    ], (error, result) => {
